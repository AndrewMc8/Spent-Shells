...
namespace StarterAssets
{
	...
	public class FirstPersonController : Character
	{
		...
		[SerializeField] protected TMP_Text ammo_text;
		...
		private void Update()
		{
			...
			if (weapon)
			{
				ammo_text.text = (weapon as Gun).RoundsRemaining + " / " + (weapon as Gun).MagCapacity + "\n" + (weapon as Gun).AmmoCount;
			}
			else
			{
				ammo_text.text = "";
        ...
        private void Shoot()
        {
			...
			if(weapon)
            {
				target.transform.position = (weapon as Gun).ViewPort.position + (weapon as Gun).ViewPort.forward;

				weapon.gameObject.transform.LookAt(target.transform.position + target.transform.forward * (weapon as Gun).Range);
            }
			...
			if (weapon is Gun && (weapon as Gun).AmmoCount <= 0 && (weapon as Gun).RoundsRemaining <= 0)
				Drop();
        }
		...
		private void CameraRotation()
		{
			...
			if (Time.timeScale > 0 && _input.look.sqrMagnitude >= _threshold)
			{
